...
;
; NAME:
;   stx_imaging_spectroscopy_electron
;
; PURPOSE: 
;   Perform imaging spectroscopy with STIX (electron space). This procedures stores the results in a folder
...
;
; CALLING SEQUENCE:
;   stx_imaging_spectroscopy_electron, path_sci_file, path_bkg_file, aux_fits_file, time_range, energy_max_inversion
;
; INPUTS:
...
;
; HISTORY:
;   largely taken over from routine stx_imaging spectroscopy_photon
...
pro stx_imaging_spectroscopy_electron, path_sci_file, path_bkg_file, aux_fits_file, time_range, energy_max_inversion,$     ; inputs
  ;; --- Optional inputs and keywords
  ...
  vis_type = 'electron visibilities'
  
  ;;;;; Define the energy range (which is later used for imaging)
  ...
  for this_en = 0,n_energies-1 do begin
    ...
    loc = where(reg_el_vis.energy_range[0] eq this_energy_range[0], count)
    vis = replicate(stx_visibility(),  count)
    vis = reg_el_vis[loc]
    

;    TO think about: some other method to stop the routine for electron maps
;    
;    This routine does not make sense for e-maps (as energies are much higher than photon maps!)
...
;    endif
    ...
    if not keyword_set(no_clean) then begin
      ...
      clean_map = stx_vis_clean(vis, aux_data, niter=niter, image_dim=imsize[0], PIXEL=pixel[0], $
        gain=gain, nmap=nmap, /plot, beam_width=beam_width, clean_box=clean_box, total_flux=max(abs(vis.obsvis)))
    endif else begin
